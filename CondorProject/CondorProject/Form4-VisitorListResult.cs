...
namespace CondorProject
{
    public partial class Form4_VisitorListResult : Form
    {
        ...
        {
            InitializeComponent();
            MessageBox.Show("load0");
            this.id = id;
            this.range = 0;
        }

        public Form4_VisitorListResult(int id, int range, string from, string to)
        {
            InitializeComponent();
        ...
        private void Form4_VisitorResult_Load(object sender, EventArgs e)
        {
           ...
           // comboBox1.SelectedIndex = 0;
            ...
            {
                MessageBox.Show("range 1");
               // MessageBox.Show(this.from);
               //MessageBox.Show(this.to);
                //visitor1TableAdapter.GetWeek(from, to);
                visitor1TableAdapter.FillWeek(condorDatabaseDataSet.Visitor1, this.from, this.to);
            }
            else if (range == 2)
            {
                MessageBox.Show("range 2");
                string[] myMonth = from.Split('/');
                visitor1TableAdapter.FillMonth(condorDatabaseDataSet.Visitor1, myMonth[0], myMonth[1]);
            }
            else if (range == 3)
            {
                MessageBox.Show("range 3");
                txtboxSearch.Text = this.from;
                //visitor1TableAdapter.GetDataBy
        ...
        private void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            {
                visitor1TableAdapter.FillDay(condorDatabaseDataSet.Visitor1, DateTime.Now.ToString("MM/dd/yyyy"));
                this.from = DateTime.Now.ToString("MM/dd/yyyy");
                this.to=DateTime.Now.ToString("MM/dd/yyyy");
            }
            else if (comboBox1.SelectedIndex == 1) //Weekly
            {
                ...
                this.from = datePicker1.Text;
                this.to = datePicker2.Text;
            ...
            else if (comboBox1.SelectedIndex == 2) //Monthly
            {
                ...
                visitor1TableAdapter.FillMonth(condorDatabaseDataSet.Visitor1, myMonth[0], myMonth[1]);
                this.from = myMonth[0] + '/' + myMonth[1];
                this.to = myMonth[0] + '/' + myMonth[1];
            }
        }
